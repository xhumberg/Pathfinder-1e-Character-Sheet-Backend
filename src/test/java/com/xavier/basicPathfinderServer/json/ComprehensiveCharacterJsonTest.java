...
class ComprehensiveCharacterJsonTest {
	...
	private static void giveHumanoidHumanType() {
		...
		Adjustment effect = AdjustmentStringConverter.convert(johnDoe, -1, name, "[Type: Humanoid (Human)][Sense: Slightly Above Average Smell]", false);
		effect.toggleAdjustment();
		johnDoe.giveRacialTrait(new RacialTrait(id, name, description, effect));
		
		Adjustment featEffect = AdjustmentStringConverter.convert(johnDoe, -1, "Custom Feat", 
